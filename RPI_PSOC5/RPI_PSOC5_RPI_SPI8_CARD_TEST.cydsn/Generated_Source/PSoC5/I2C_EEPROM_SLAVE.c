...
volatile uint8 I2C_EEPROM_slStatus;   /* Slave Status  */

/* Transmit buffer variables */
volatile uint8 * I2C_EEPROM_slRdBufPtr;   /* Pointer to Transmit buffer  */
volatile uint8   I2C_EEPROM_slRdBufSize;  /* Slave Transmit buffer size  */
volatile uint8   I2C_EEPROM_slRdBufIndex; /* Slave Transmit buffer Index */

/* Receive buffer variables */
volatile uint8 * I2C_EEPROM_slWrBufPtr;   /* Pointer to Receive buffer  */
volatile uint8   I2C_EEPROM_slWrBufSize;  /* Slave Receive buffer size  */
volatile uint8   I2C_EEPROM_slWrBufIndex; /* Slave Receive buffer Index */

#if (I2C_EEPROM_SW_ADRR_DECODE)
    volatile uint8 I2C_EEPROM_slAddress;  /* Software address variable */
#endif /* (I2C_EEPROM_SW_ADRR_DECODE) */


/*******************************************************************************
* Function Name: I2C_EEPROM_SlaveStatus
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2C_EEPROM_slStatus - The global variable used to store a current
*                              status of the I2C slave.
*
*******************************************************************************/
uint8 I2C_EEPROM_SlaveStatus(void) 
{
    return (I2C_EEPROM_slStatus);
}


/*******************************************************************************
* Function Name: I2C_EEPROM_SlaveClearReadStatus
********************************************************************************
*
* Summary:
*  Clears the read status flags and returns they values.
*  The I2C_EEPROM_SSTAT_RD_BUSY flag is not effected by clear.
*
* Parameters:
*  None.
*
...
uint8 I2C_EEPROM_SlaveClearReadStatus(void) 
{
    ...
    status = (I2C_EEPROM_slStatus & I2C_EEPROM_SSTAT_RD_MASK);
    I2C_EEPROM_slStatus &= (uint8) ~I2C_EEPROM_SSTAT_RD_CLEAR;
...
/*******************************************************************************
* Function Name: I2C_EEPROM_SlaveClearWriteStatus
********************************************************************************
*
* Summary:
*  Clears the write status flags and returns they values.
*  The I2C_EEPROM_SSTAT_WR_BUSY flag is not effected by clear.
*
* Parameters:
*  None.
*
...
uint8 I2C_EEPROM_SlaveClearWriteStatus(void) 
{
    ...
    /* Mask of transfer complete flag and Error status */
    status = (I2C_EEPROM_slStatus & I2C_EEPROM_SSTAT_WR_MASK);
    I2C_EEPROM_slStatus &= (uint8) ~I2C_EEPROM_SSTAT_WR_CLEAR;
...
/*******************************************************************************
* Function Name: I2C_EEPROM_SlaveSetAddress
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2C_EEPROM_Address  - The global variable used to store an I2C slave
...
{
#if (I2C_EEPROM_SW_ADRR_DECODE)
    I2C_EEPROM_slAddress = (address & I2C_EEPROM_SLAVE_ADDR_MASK);
#else
    I2C_EEPROM_ADDR_REG  = (address & I2C_EEPROM_SLAVE_ADDR_MASK);
#endif /* (I2C_EEPROM_SW_ADRR_DECODE) */
}


/*******************************************************************************
* Function Name: I2C_EEPROM_SlaveInitReadBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2C_EEPROM_slRdBufPtr   - The global variable used to store a pointer
*                                  to the slave read buffer.
*  I2C_EEPROM_slRdBufSize  - The global variable used to store a slave
*                                  read buffer size.
*  I2C_EEPROM_slRdBufIndex - The global variable used to store a current
     ...
     
{
    if (NULL != rdBuf)
    {
        ...
        /* Set buffer pointer */
        I2C_EEPROM_slRdBufPtr   = (volatile uint8 *) rdBuf;
        I2C_EEPROM_slRdBufSize  = bufSize;    /* Set buffer size     */
        I2C_EEPROM_slRdBufIndex = 0u;         /* Clears buffer index */
...
/*******************************************************************************
* Function Name: I2C_EEPROM_SlaveInitWriteBuf
********************************************************************************
*
* Summary:
*  Sets the buffer pointer and size of the read buffer. This function also
...
*
* Global variables:
*  I2C_EEPROM_slWrBufPtr   - The global variable used to store a pointer
*                                  to the slave write buffer.
*  I2C_EEPROM_slWrBufSize  - The global variable used to store a slave
*                                  write buffer size.
*  I2C_EEPROM_slWrBufIndex - The global variable used to store a current
     ...
     
{
    if (NULL != wrBuf)
    {
        I2C_EEPROM_DisableInt(); /* Lock from interrupt */
         ...
         /* Set buffer pointer */
        I2C_EEPROM_slWrBufPtr   = (volatile uint8 *) wrBuf;
        I2C_EEPROM_slWrBufSize  = bufSize;    /* Set buffer size     */
        I2C_EEPROM_slWrBufIndex = 0u;         /* Clears buffer index */
...
/*******************************************************************************
* Function Name: I2C_EEPROM_SlaveGetReadBufSize
********************************************************************************
*
* Summary:
...
{
    return (I2C_EEPROM_slRdBufIndex);
}


/*******************************************************************************
* Function Name: I2C_EEPROM_SlaveGetWriteBufSize
********************************************************************************
*
* Summary:
...
{
    return (I2C_EEPROM_slWrBufIndex);
}


/*******************************************************************************
* Function Name: I2C_EEPROM_SlaveClearReadBuf
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2C_EEPROM_SlaveClearRxBuf
********************************************************************************
*
* Summary:
