...
I2C1_BACKUP_STRUCT I2C1_backup =
{
    I2C1_DISABLE,
    ...
    I2C1_DEFAULT_CFG,
    I2C1_DEFAULT_ADDR,
    LO8(I2C1_DEFAULT_DIVIDE_FACTOR),
    HI8(I2C1_DEFAULT_DIVIDE_FACTOR),
#else  /* (I2C1_UDB_IMPLEMENTED) */
    I2C1_DEFAULT_CFG,
#endif /* (I2C1_FF_IMPLEMENTED) */

#if (I2C1_TIMEOUT_ENABLED)
    I2C1_DEFAULT_TMOUT_PERIOD,
    I2C1_DEFAULT_TMOUT_INTR_MASK,
#endif /* (I2C1_TIMEOUT_ENABLED) */
};

#if ((I2C1_FF_IMPLEMENTED) && (I2C1_WAKEUP_ENABLED))
    volatile uint8 I2C1_wakeupSource;
#endif /* ((I2C1_FF_IMPLEMENTED) && (I2C1_WAKEUP_ENABLED)) */


/*******************************************************************************
* Function Name: I2C1_SaveConfig
********************************************************************************
*
* Summary:
...
*
* Global Variables:
*  I2C1_backup - The global variable used to save the component
...
{
#if (I2C1_FF_IMPLEMENTED)
    #if (I2C1_WAKEUP_ENABLED)
        uint8 intState;
    #endif /* (I2C1_WAKEUP_ENABLED) */

    /* Store registers before enter low power mode */
    I2C1_backup.cfg     = I2C1_CFG_REG;
    I2C1_backup.xcfg    = I2C1_XCFG_REG;
    I2C1_backup.addr    = I2C1_ADDR_REG;
    I2C1_backup.clkDiv1 = I2C1_CLKDIV1_REG;
    I2C1_backup.clkDiv2 = I2C1_CLKDIV2_REG;

#if (I2C1_WAKEUP_ENABLED)
    /* Disable master */
    I2C1_CFG_REG &= (uint8) ~I2C1_ENABLE_MASTER;

    /* Enable backup regulator to keep block powered in low power mode */
    intState = CyEnterCriticalSection();
    I2C1_PWRSYS_CR1_REG |= I2C1_PWRSYS_CR1_I2C_REG_BACKUP;
    ...
    */
    ...
    I2C1_XCFG_REG |= I2C1_XCFG_FORCE_NACK;
    while (0u == (I2C1_XCFG_REG & I2C1_XCFG_RDY_TO_SLEEP))
    {
    }

    /* Setup wakeup interrupt */
    I2C1_DisableInt();
    (void) CyIntSetVector(I2C1_ISR_NUMBER, &I2C1_WAKEUP_ISR);
    I2C1_wakeupSource = 0u;
    I2C1_EnableInt();
#endif /* (I2C1_WAKEUP_ENABLED) */

#else
    /* Store only address match bit */
    I2C1_backup.control = (I2C1_CFG_REG & I2C1_CTRL_ANY_ADDRESS_MASK);
#endif /* (I2C1_FF_IMPLEMENTED) */

#if (I2C1_TIMEOUT_ENABLED)
    I2C1_TimeoutSaveConfig();
#endif /* (I2C1_TIMEOUT_ENABLED) */
}


/*******************************************************************************
* Function Name: I2C1_Sleep
********************************************************************************
*
* Summary:
...
void I2C1_Sleep(void) 
{
    ...
    I2C1_backup.enableState = I2C1_DISABLE;
...
#else
    /* Store enable state */
    I2C1_backup.enableState = (uint8) I2C1_IS_ENABLED;

    if (0u != I2C1_backup.enableState)
    {
        I2C1_Stop();
    }
#endif /* (I2C1_WAKEUP_ENABLED) */
...
/*******************************************************************************
* Function Name: I2C1_RestoreConfig
********************************************************************************
*
* Summary:
*  The Enable wakeup from Sleep Mode selection influences this function
...
*
* Side Effects:
*  Calling this function before I2C1_SaveConfig() or
*  I2C1_Sleep() will lead to unpredictable results.
*
*******************************************************************************/
void I2C1_RestoreConfig(void) 
{
    ...
    if (I2C1_CHECK_PWRSYS_I2C_BACKUP)
    /* Low power mode was Sleep - backup regulator is enabled */
    {
        /* Enable backup regulator in active mode */
        intState = CyEnterCriticalSection();
        I2C1_PWRSYS_CR1_REG &= (uint8) ~I2C1_PWRSYS_CR1_I2C_REG_BACKUP;
        CyExitCriticalSection(intState);

        /* Restore master */
        I2C1_CFG_REG = I2C1_backup.cfg;
    }
    else
    /* Low power mode was Hibernate - backup regulator is disabled. All registers are cleared */
    {
        ...
        intState = CyEnterCriticalSection();
        I2C1_ACT_PWRMGR_REG  &= (uint8) ~I2C1_ACT_PWR_EN;
        I2C1_STBY_PWRMGR_REG &= (uint8) ~I2C1_STBY_PWR_EN;
        CyExitCriticalSection(intState);

        /* Enable component in I2C_Wakeup() after register restore */
        I2C1_backup.enableState = I2C1_ENABLE;
    #endif /* (I2C1_WAKEUP_ENABLED) */

        /* Restore component registers after Hibernate */
        I2C1_XCFG_REG    = I2C1_backup.xcfg;
        I2C1_CFG_REG     = I2C1_backup.cfg;
        I2C1_ADDR_REG    = I2C1_backup.addr;
        I2C1_CLKDIV1_REG = I2C1_backup.clkDiv1;
        I2C1_CLKDIV2_REG = I2C1_backup.clkDiv2;
    }

#if (I2C1_WAKEUP_ENABLED)
    I2C1_DisableInt();
    (void) CyIntSetVector(I2C1_ISR_NUMBER, &I2C1_ISR);
    if (0u != I2C1_wakeupSource)
    {
...
#else
    I2C1_CFG_REG = I2C1_backup.control;
#endif /* (I2C1_FF_IMPLEMENTED) */

#if (I2C1_TIMEOUT_ENABLED)
    I2C1_TimeoutRestoreConfig();
#endif /* (I2C1_TIMEOUT_ENABLED) */
}


/*******************************************************************************
* Function Name: I2C1_Wakeup
********************************************************************************
*
* Summary:
